...
DROP PROCEDURE IF EXISTS `GetBriefCount`;
...
BEGIN
	...
	WHERE (parent_ID=parentID OR (brief_id=parentID and parent_ID=0)) AND `status` > 0;
    ...
    SELECT brief_name INTO @last_brief_name
		FROM project_brief WHERE parent_id=parentID AND version=@version-1;
...
DELIMITER $$
CREATE PROCEDURE `GetBriefCount`(
    IN `userIDIN` INT,
	IN `clientIDIN` INT
)
BEGIN
	...
	IF clientIDIN IS NOT NULL AND clientIDIN <> 0 THEN
		-- get check status 
		SELECT COUNT(brief_id) INTO draft FROM project_brief WHERE status=1 AND client_id=clientIDIN;
		SELECT COUNT(brief_id) INTO submit FROM project_brief WHERE status=2 AND client_id=clientIDIN;
		SELECT COUNT(brief_id) INTO inProgress FROM project_brief WHERE status=3 AND client_id=clientIDIN;
		SELECT COUNT(brief_id) INTO completed FROM project_brief WHERE status=8 AND client_id=clientIDIN;
		SELECT COUNT(brief_id) INTO cancel FROM project_brief WHERE status=9 AND client_id=clientIDIN;
					
		SELECT draft, submit, inProgress, completed, cancel; 

		-- get modify 
		SELECT COUNT(brief_id) INTO draft FROM project_brief WHERE status=1 AND client_id=clientIDIN AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO submit FROM project_brief WHERE status=2 AND client_id=clientIDIN AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO inProgress FROM project_brief WHERE status=3 AND client_id=clientIDIN AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO completed FROM project_brief WHERE status=8 AND client_id=clientIDIN AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO cancel FROM project_brief WHERE status=9 AND client_id=clientIDIN AND user_id=userIDIN;
		
		SELECT draft, submit, inProgress, completed, cancel; 
	ELSE
		DROP TEMPORARY TABLE IF EXISTS tmp_client;
		CREATE TEMPORARY TABLE tmp_client (client_id INT(11) NOT NULL);
		
		INSERT INTO tmp_client 
		SELECT c.ClientID FROM `User` u LEFT JOIN UserClientMap ucm ON u.UserID = ucm.UserID
			LEFT JOIN Client c ON ucm.ClientID=c.ClientID WHERE u.UserID = userIDIN;
		-- get check status 
		SELECT COUNT(brief_id) INTO draft FROM project_brief pb
			INNER JOIN tmp_client tc ON pb.client_id=tc.client_id 
			WHERE status=1;
		SELECT COUNT(brief_id) INTO submit FROM project_brief pb
			INNER JOIN tmp_client tc ON pb.client_id=tc.client_id 
			WHERE status=2;
		SELECT COUNT(brief_id) INTO inProgress FROM project_brief pb
			INNER JOIN tmp_client tc ON pb.client_id=tc.client_id 
			WHERE status=3;
		SELECT COUNT(brief_id) INTO completed FROM project_brief pb
			INNER JOIN tmp_client tc ON pb.client_id=tc.client_id 
			WHERE status=8;
		SELECT COUNT(brief_id) INTO cancel FROM project_brief pb
			INNER JOIN tmp_client tc ON pb.client_id=tc.client_id 
			WHERE status=9;
		...
		-- get modify 
		SELECT COUNT(brief_id) INTO draft FROM project_brief WHERE status=1 AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO submit FROM project_brief WHERE status=2 AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO inProgress FROM project_brief WHERE status=3 AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO completed FROM project_brief WHERE status=8 AND user_id=userIDIN;
		SELECT COUNT(brief_id) INTO cancel FROM project_brief WHERE status=9 AND user_id=userIDIN;
...
BEGIN
  ...
  set @final_sql = '';
	...
	IF @role_name ='AccountTeam' THEN
		...
		IF clientIDIN = 0 OR clientIDIN IS NULL THEN
			-- get user's client
		...
		ELSE 
				...
				IF isModify = 1 OR isModify IS NULL THEN
					...
					WHERE p.user_id = ',userIDIN,' AND p.client_id = ',clientIDIN,' AND p.`status` > 0');
